...
void EditTransform(float *model, bool editTransformDecomposition) {
  ...
  constexpr auto kTranslateKey = 'Z';
  constexpr auto kRotateKey = 'E';
  constexpr auto kScaleKey = 'R';
  constexpr auto kSnapKey = 'S';
  ...
  if (editTransformDecomposition) {
    ...
    if (ImGui::RadioButton("Rotate",
      ...
      gizmoFlags = ImGuizmoOperationFlags_Rotate;
    ImGui::SameLine();
    if (ImGui::RadioButton("Scale", gizmoFlags == ImGuizmoOperationFlags_Scale))
      gizmoFlags = ImGuizmoOperationFlags_Scale;
    ...
    switch (gizmoFlags) {
    case ImGuizmoOperationFlags_Translate:
      ImGui::InputFloat3("Snap", &snap[0]);
      break;
    case ImGuizmoOperationFlags_Rotate:
      ImGui::InputFloat("Angle Snap", &snap[0]);
      break;
    case ImGuizmoOperationFlags_Scale:
...
int main(int, char **) {
  ...
  HWND hwnd = ::CreateWindow(
    wc.lpszClassName, _T("Dear ImGui DirectX12 Example"), WS_OVERLAPPEDWINDOW,
  ...
  while (msg.message != WM_QUIT) {
    ...
    if (ImGui::IsKeyPressed(90)) gizmoEnabled = !gizmoEnabled;
    ...
    ImGuizmo::CreateCanvas("ImGuizmo", mainViewport->Pos, mainViewport->Size);
    ...
    const glm::vec2 position{ glm::vec2{ ImGui::GetWindowPos() } +
                              glm::vec2{ 0, 20 } };
    ImGuizmo::ViewManipulate(glm::value_ptr(cameraView), camDistance, position,
...
bool CreateDeviceD3D(HWND hWnd) {
...

  {
    ...
    if (CreateDXGIFactory1(IID_PPV_ARGS(&dxgiFactory)) != S_OK ||
        dxgiFactory->CreateSwapChainForHwnd(g_pd3dCommandQueue, hWnd, &sd, NULL,
