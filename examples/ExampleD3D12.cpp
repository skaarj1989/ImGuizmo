...
int main(int, char**)
{
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        if (isPerspective) {
          cameraProjection = glm::perspective(
            glm::radians(fov), io.DisplaySize.x / io.DisplaySize.y, 0.1f,
            1000.0f);
        } else {
          const float viewHeight{ viewWidth * io.DisplaySize.y /
                                  io.DisplaySize.x };
    ...
    const ImGuiViewport *mainViewport{ ImGui::GetMainViewport() };
        ImGuizmo::SetupWorkspace("ImGuizmo", mainViewport->Pos,
                                 mainViewport->Size);
