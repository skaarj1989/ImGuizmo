#pragma once

// http://devernay.free.fr/cours/opengl/materials.html
struct material_t {
  //const char *name{ nullptr };
  glm::vec3 Ka, Kd, Ks;
  float Shininess;
};

std::unordered_map<std::string, material_t> MaterialPresets{
  { "Emerald", { glm::vec3{ 0.0215f, 0.1745f, 0.0215f }, glm::vec3{ 0.07568f, 0.61424f, 0.07568f }, glm::vec3{ 0.633, 0.727811f, 0.633f }, 0.6f } },
  { "Jade", { glm::vec3{ 0.135f, 0.2225f, 0.1575f }, glm::vec3{ 0.54f, 0.89f, 0.63f }, glm::vec3{ 0.316228f }, 0.1f } },
  { "Obsidian", { glm::vec3{ 0.05375f, 0.05f, 0.06625f }, glm::vec3{ 0.18275f, 0.17f, 0.22525f }, glm::vec3{ 0.332741f, 0.328634f, 0.346435f }, 0.3f } },
  { "Pearl", { glm::vec3{ 0.25f, 0.20725f, 0.20725f }, glm::vec3{ 1.0f, 0.829f, 0.829f }, glm::vec3{ 0.296648f }, 0.088f } },
  { "Ruby", { glm::vec3{ 0.1745f, 0.01175f, 0.01175f }, glm::vec3{ 0.61424f, 0.04136f, 0.04136f }, glm::vec3{ 0.727811f, 0.626959f, 0.626959f }, 0.6f } },
  { "Turquoise", { glm::vec3{ 0.1f, 0.18725f, 0.1745f }, glm::vec3{ 0.396f, 0.74151f, 0.69102f }, glm::vec3{ 0.297254f, 0.30829f, 0.306678f }, 0.1f } },
  { "Brass", { glm::vec3{ 0.329412f, 0.223529f, 0.027451f }, glm::vec3{ 0.780392f, 0.568627f, 0.113725f }, glm::vec3{ 0.992157f, 0.941176f, 0.807843f }, 0.21794872f } },
  { "Bronze", { glm::vec3{ 0.2125f, 0.1275f, 0.054f }, glm::vec3{ 0.714f, 0.4284f, 0.18144f }, glm::vec3{ 0.393548f, 0.271906f, 0.166721f }, 0.2f } },
  { "Chrome", { glm::vec3{ 0.25f },  glm::vec3{ 0.4f }, glm::vec3{ 0.774597f }, 0.6f } },
  { "Copper", { glm::vec3{ 0.19125f, 0.0735f, 0.0225f }, glm::vec3{ 0.7038f, 0.27048f, 0.0828f }, glm::vec3{ 0.256777f, 0.137622f, 0.086014f }, 0.1f } },
  { "Gold", { glm::vec3{ 0.24725f, 0.1995f, 0.0745f }, glm::vec3{ 0.75164f, 0.60648f, 0.22648f }, glm::vec3{ 0.628281f, 0.555802f, 0.366065f }, 0.4f } },
  { "Silver", { glm::vec3{ 0.19225f }, glm::vec3{ 0.50754f }, glm::vec3{ 0.508273f }, 0.4f } },
  { "BlackPlastic", { glm::vec3{ 0.0f }, glm::vec3{ 0.01f }, glm::vec3{ 0.50f }, 0.25f } },
  { "CyanPlastic", { glm::vec3{ 0.0f, 0.1f, 0.06f }, glm::vec3{ 0.0f, 0.50980392f, 0.50980392f }, glm::vec3{ 0.50196078f, 0.50196078f, 0.50196078f }, 0.25f } },
  { "GreenPlastic", { glm::vec3{ 0.0f }, glm::vec3{ 0.1f, 0.35f, 0.1f }, glm::vec3{ 0.45f, 0.55f, 0.45f }, 0.25f } },
  { "RedPlastic", { glm::vec3{ 0.0f }, glm::vec3{ 0.5f, 0.0f, 0.0f }, glm::vec3{ 0.7f, 0.6f, 0.6f }, 0.25f } },
  { "WhitePlastic", { glm::vec3{ 0.0f }, glm::vec3{ 0.55f }, glm::vec3{ 0.70f }, 0.25f } },
  { "YellowPlastic", { glm::vec3{ 0.0f }, glm::vec3{ 0.5f, 0.5f, 0.0f }, glm::vec3{ 0.60f, 0.60f, 0.50f }, 0.25f } },
  { "BlackRubber", { glm::vec3{ 0.02f }, glm::vec3{ 0.01f }, glm::vec3{ 0.4f }, 0.078125f } },
  { "CyanRubber", { glm::vec3{ 0.0f, 0.05f, 0.05f }, glm::vec3{ 0.4f, 0.5f, 0.5f }, glm::vec3{ 0.04f, 0.7f, 0.7f }, 0.078125f } },
  { "GreenRubber", { glm::vec3{ 0.0f, 0.05f, 0.0f }, glm::vec3{ 0.4f, 0.5f, 0.4f }, glm::vec3{ 0.04f, 0.7f, 0.04f }, 0.078125f } },
  { "RedRubber", { glm::vec3{ 0.05f, 0.0f, 0.0f }, glm::vec3{ 0.5f, 0.4f, 0.4f }, glm::vec3{ 0.7f, 0.04f, 0.04f }, 0.078125f } }, 
  { "WhiteRubber", { glm::vec3{ 0.05f }, glm::vec3{ 0.05f }, glm::vec3{ 0.7f }, 0.078125f } },
  { "YelowRubber", { glm::vec3{ 0.05f, 0.05f, 0.0f }, glm::vec3{ 0.5f, 0.5f, 0.4f }, glm::vec3{ 0.7f, 0.7f, 0.04f }, 0.078125f } }
};

#if 0
std::array MaterialPresets{
  material_t {
    "Emerald",
    glm::vec3{ 0.0215f, 0.1745f, 0.0215f },
    glm::vec3{ 0.07568f, 0.61424f, 0.07568f },
    glm::vec3{ 0.633, 0.727811f, 0.633f },
    0.6f
  },
  material_t {
    "Jade",
    glm::vec3{ 0.135f, 0.2225f, 0.1575f },
    glm::vec3{ 0.54f, 0.89f, 0.63f },
    glm::vec3{ 0.316228f },
    0.1f
  },
  material_t {
    "Obsidian",
    glm::vec3{ 0.05375f, 0.05f, 0.06625f },
    glm::vec3{ 0.18275f, 0.17f, 0.22525f },
    glm::vec3{ 0.332741f, 0.328634f, 0.346435f },
    0.3f
  },
  material_t {
    "Pearl",
    glm::vec3{ 0.25f, 0.20725f, 0.20725f },
    glm::vec3{ 1.0f, 0.829f, 0.829f },
    glm::vec3{ 0.296648f },
    0.088f
  },
  material_t {
    "Ruby",
    glm::vec3{ 0.1745f, 0.01175f, 0.01175f },
    glm::vec3{ 0.61424f, 0.04136f, 0.04136f },
    glm::vec3{ 0.727811f, 0.626959f, 0.626959f },
    0.6f
  },
  material_t {
    "Turquoise",
    glm::vec3{ 0.1f, 0.18725f, 0.1745f },
    glm::vec3{ 0.396f, 0.74151f, 0.69102f },
    glm::vec3{ 0.297254f, 0.30829f, 0.306678f },
    0.1f
  },
   
  material_t {
    "Brass",
    glm::vec3{ 0.329412f, 0.223529f, 0.027451f },
    glm::vec3{ 0.780392f, 0.568627f, 0.113725f },
    glm::vec3{ 0.992157f, 0.941176f, 0.807843f },
    0.21794872f
  },
  material_t {
    "Bronze",
    glm::vec3{ 0.2125f, 0.1275f, 0.054f },
    glm::vec3{ 0.714f, 0.4284f, 0.18144f },
    glm::vec3{ 0.393548f, 0.271906f, 0.166721f },
    0.2f
  },
  material_t {
    "Chrome",
    glm::vec3{ 0.25f },
    glm::vec3{ 0.4f },
    glm::vec3{ 0.774597f },
    0.6f
  },
  material_t {
    "Copper",
    glm::vec3{ 0.19125f, 0.0735f, 0.0225f },
    glm::vec3{ 0.7038f, 0.27048f, 0.0828f },
    glm::vec3{ 0.256777f, 0.137622f, 0.086014f },
    0.1f
  },
  material_t {
    "Gold",
    glm::vec3{ 0.24725f, 0.1995f, 0.0745f },
    glm::vec3{ 0.75164f, 0.60648f, 0.22648f },
    glm::vec3{ 0.628281f, 0.555802f, 0.366065f },
    0.4f
  },
  material_t {
    "Silver",
    glm::vec3{ 0.19225f },
    glm::vec3{ 0.50754f	},
    glm::vec3{ 0.508273f },
    0.4f
  },
  
  material_t {
    "BlackPlastic",
    glm::vec3{ 0.0f },
    glm::vec3{ 0.01f },
    glm::vec3{ 0.50f },
    0.25f
  },
  material_t {
    "CyanPlastic",
    glm::vec3{ 0.0f, 0.1f, 0.06f },
    glm::vec3{ 0.0f, 0.50980392f, 0.50980392f },
    glm::vec3{ 0.50196078f, 0.50196078f, 0.50196078f },
    0.25f
  },
  material_t {
    "GreenPlastic",
    glm::vec3{ 0.0f },
    glm::vec3{ 0.1f, 0.35f, 0.1f },
    glm::vec3{ 0.45f, 0.55f, 0.45f },
    0.25f
  },
  material_t {
    "RedPlastic",
    glm::vec3{ 0.0f },
    glm::vec3{ 0.5f, 0.0f, 0.0f },
    glm::vec3{ 0.7f, 0.6f, 0.6f },
    0.25f
  },
  material_t {
    "WhitePlastic",
    glm::vec3{ 0.0f },
    glm::vec3{ 0.55f },
    glm::vec3{ 0.70f },
    0.25f
  },
  material_t {
    "YellowPlastic",
    glm::vec3{ 0.0f },
    glm::vec3{ 0.5f, 0.5f, 0.0f },
    glm::vec3{ 0.60f, 0.60f, 0.50f },
    0.25f
  },
  
  material_t { 
    "BlackRubber",
    glm::vec3{ 0.02f },
    glm::vec3{ 0.01f },
    glm::vec3{ 0.4f },
    0.078125f
  },
  material_t {
    "CyanRubber",
    glm::vec3{ 0.0f, 0.05f, 0.05f },
    glm::vec3{ 0.4f, 0.5f, 0.5f },
    glm::vec3{ 0.04f, 0.7f, 0.7f },
    0.078125f
  },
  material_t {
    "GreenRubber",
    glm::vec3{ 0.0f, 0.05f, 0.0f },
    glm::vec3{ 0.4f, 0.5f, 0.4f },
    glm::vec3{ 0.04f, 0.7f, 0.04f },
    0.078125f
  },
  material_t {
    "RedRubber",
    glm::vec3{ 0.05f, 0.0f, 0.0f },
    glm::vec3{ 0.5f, 0.4f, 0.4f },
    glm::vec3{ 0.7f, 0.04f, 0.04f },
    0.078125f
  },
  material_t {
    "WhiteRubber",
    glm::vec3{ 0.05f },
    glm::vec3{ 0.05f },
    glm::vec3{ 0.7f },
    0.078125f
  },
  material_t {
    "YelowRubber",
    glm::vec3{ 0.05f, 0.05f, 0.0f },
    glm::vec3{ 0.5f, 0.5f, 0.4f },
    glm::vec3{ 0.7f, 0.7f, 0.04f },
    0.078125f
  }
};
#elif 0
struct {
  const material_t Emerald {
    "Emerald",
    glm::vec3{ 0.0215f, 0.1745f, 0.0215f },
    glm::vec3{ 0.07568f, 0.61424f, 0.07568f },
    glm::vec3{ 0.633, 0.727811f, 0.633f },
    0.6f
  };
  const material_t Jade {
    "Jade",
    glm::vec3{ 0.135f, 0.2225f, 0.1575f },
    glm::vec3{ 0.54f, 0.89f, 0.63f },
    glm::vec3{ 0.316228f },
    0.1f
  };
  const material_t Obsidian {
    "Obsidian",
    glm::vec3{ 0.05375f, 0.05f, 0.06625f },
    glm::vec3{ 0.18275f, 0.17f, 0.22525f },
    glm::vec3{ 0.332741f, 0.328634f, 0.346435f },
    0.3f
  };
  const material_t Pearl {
    "Pearl",
    glm::vec3{ 0.25f, 0.20725f, 0.20725f },
    glm::vec3{ 1.0f, 0.829f, 0.829f },
    glm::vec3{ 0.296648f },
    0.088f
  };
  const material_t Ruby{
    "Ruby",
    glm::vec3{ 0.1745f, 0.01175f, 0.01175f },
    glm::vec3{ 0.61424f, 0.04136f, 0.04136f },
    glm::vec3{ 0.727811f, 0.626959f, 0.626959f },
    0.6f
  };
  const material_t Turquoise{
    "Turquoise",
    glm::vec3{ 0.1f, 0.18725f, 0.1745f },
    glm::vec3{ 0.396f, 0.74151f, 0.69102f },
    glm::vec3{ 0.297254f, 0.30829f, 0.306678f },
    0.1f
  };
   
  const material_t Brass{
    "Brass",
    glm::vec3{ 0.329412f, 0.223529f, 0.027451f },
    glm::vec3{ 0.780392f, 0.568627f, 0.113725f },
    glm::vec3{ 0.992157f, 0.941176f, 0.807843f },
    0.21794872f
  };
  const material_t Bronze {
    "Bronze",
    glm::vec3{ 0.2125f, 0.1275f, 0.054f },
    glm::vec3{ 0.714f, 0.4284f, 0.18144f },
    glm::vec3{ 0.393548f, 0.271906f, 0.166721f },
    0.2f
  };
  const material_t Chrome {
    "Chrome",
    glm::vec3{ 0.25f },
    glm::vec3{ 0.4f },
    glm::vec3{ 0.774597f },
    0.6f
  };
  const material_t Copper {
    "Copper",
    glm::vec3{ 0.19125f, 0.0735f, 0.0225f },
    glm::vec3{ 0.7038f, 0.27048f, 0.0828f },
    glm::vec3{ 0.256777f, 0.137622f, 0.086014f },
    0.1f
  };
  const material_t Gold {
    "Gold",
    glm::vec3{ 0.24725f, 0.1995f, 0.0745f },
    glm::vec3{ 0.75164f, 0.60648f, 0.22648f },
    glm::vec3{ 0.628281f, 0.555802f, 0.366065f },
    0.4f
  };
  const material_t Silver {
    "Silver",
    glm::vec3{ 0.19225f },
    glm::vec3{ 0.50754f	},
    glm::vec3{ 0.508273f },
    0.4f
  };
  
  const material_t BlackPlastic {
    "BlackPlastic",
    glm::vec3{ 0.0f },
    glm::vec3{ 0.01f },
    glm::vec3{ 0.50f },
    0.25f
  };
  const material_t CyanPlastic {
    "CyanPlastic",
    glm::vec3{ 0.0f, 0.1f, 0.06f },
    glm::vec3{ 0.0f, 0.50980392f, 0.50980392f },
    glm::vec3{ 0.50196078f, 0.50196078f, 0.50196078f },
    0.25f
  };
  const material_t GreenPlastic {
    "GreenPlastic",
    glm::vec3{ 0.0f },
    glm::vec3{ 0.1f, 0.35f, 0.1f },
    glm::vec3{ 0.45f, 0.55f, 0.45f },
    0.25f
  };
  const material_t RedPlastic {
    "RedPlastic",
    glm::vec3{ 0.0f },
    glm::vec3{ 0.5f, 0.0f, 0.0f },
    glm::vec3{ 0.7f, 0.6f, 0.6f },
    0.25f
  };
  const material_t WhitePlastic {
    "WhitePlastic",
    glm::vec3{ 0.0f },
    glm::vec3{ 0.55f },
    glm::vec3{ 0.70f },
    0.25f
  };
  const material_t YellowPlastic {
    "YellowPlastic",
    glm::vec3{ 0.0f },
    glm::vec3{ 0.5f, 0.5f, 0.0f },
    glm::vec3{ 0.60f, 0.60f, 0.50f },
    0.25f
  };
  
  const material_t BlackRubber { 
    "BlackRubber",
    glm::vec3{ 0.02f },
    glm::vec3{ 0.01f },
    glm::vec3{ 0.4f },
    0.078125f
  };
  const material_t CyanRubber {
    "CyanRubber",
    glm::vec3{ 0.0f, 0.05f, 0.05f },
    glm::vec3{ 0.4f, 0.5f, 0.5f },
    glm::vec3{ 0.04f, 0.7f, 0.7f },
    0.078125f
  };
  const material_t GreenRubber {
    "GreenRubber",
    glm::vec3{ 0.0f, 0.05f, 0.0f },
    glm::vec3{ 0.4f, 0.5f, 0.4f },
    glm::vec3{ 0.04f, 0.7f, 0.04f },
    0.078125f
  };
const material_t RedRubber{
    "RedRubber",
    glm::vec3{ 0.05f, 0.0f, 0.0f },
    glm::vec3{ 0.5f, 0.4f, 0.4f },
    glm::vec3{ 0.7f, 0.04f, 0.04f },
    0.078125f
  };
  const material_t WhiteRubber {
    "WhiteRubber",
    glm::vec3{ 0.05f },
    glm::vec3{ 0.05f },
    glm::vec3{ 0.7f },
    0.078125f
  };
  const material_t YelowRubber {
    "YelowRubber",
    glm::vec3{ 0.05f, 0.05f, 0.0f },
    glm::vec3{ 0.5f, 0.5f, 0.4f },
    glm::vec3{ 0.7f, 0.7f, 0.04f },
    0.078125f
  };
} MaterialPrototypes;
#endif